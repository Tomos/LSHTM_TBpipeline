    ...
    for (zz in 1:length(qr_S)){
      ...
      for (bb in 1:length(beta_r)){
        ...
        for (ww in 1:length(w_r)){
          ...
          if (ii==3){ 
            source("SS_3.R")
            state <- c(S=S,L=L,I=I,P=0,N_PT=0, TT=0)
            #print(state)
...
saveRDS(Figure_A2, paste0("Figure_2A_mod.rds"))
...
saveRDS(Figure_2, paste0("Figure_2_mod.rds"))
...
saveRDS(Figure_3, paste0("Figure_3_mod.rds"))
